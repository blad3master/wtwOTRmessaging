...
**
** Remarks:
**		Do not use WTW_NETLIB_FLAG_SSL when creating socket
**
*/

#define WTW_NETLIB_INITSRVSSL	L"NetLib/InitSRVSSL"

struct netLibServerSSL
{
	__wtwStdStructDefs(netLibServerSSL);

	int				structSize;

	const wchar_t * sockId;
	const wchar_t *	pemFile;	// pem certificate location, this can be NULL
								// if it's NULL then cert will be stored at wtw's cache
								// directory as "<cn>.pem"
								// if it's not NULL it must be fully qualified path

	const wchar_t *	cn;			// server common name, must be valid
	const wchar_t *	email;		// can be NULL 

	DWORD			flags;		// NL_SSL_SERVER_*
	DWORD			mechs;		// NL_SSL_MECH_*

	void		*	reserved;	//
};

#define NL_SSL_SERVER_NO_AUTO_GEN	0x00000001	// don't automatically generate cert if not exists
												// just return error (E_NOT_SET)
...
#define WTW_NETLIB_PROXYTYPE_NONE		0x00
...
#define WTW_NETLIB_FLAG_SSL			0x00000002	// Enable TLS layer, call SSLINIT to send client handshake 
#define WTW_NETLIB_FLAG_LISTEN		0x00000004	// Listening socket
#define WTW_NETLIB_FLAG_ACCEPT		0x00000010	// Accept incoming connection, "fromSocket" must be filled
#define WTW_NETLIB_FLAG_TLSA_CHECK	0x00000020	// Do a TLSA record check, used only when TLS is enabled
...
#define WTW_SSL_FAILURE_USER_WAIT	5 
