...
_gcry_initialize_fips_mode (int force)
{
  ...
  // blad3master: remove the FIPS functionality
    ...
    if (fp)
      {
        ...
        fclose (fp);
             ...
             && saved_errno != EACCES
             //&& !access ("/proc/version", F_OK) blad3master: remove this condition as it is not checked anayway but would introduce F_OK type
			 )
      {
